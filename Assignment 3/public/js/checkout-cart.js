  ...
  cart.forEach(item => {
    ...
    li.innerHTML = `
      <div class="d-flex align-items-center gap-2">
        <button class="btn btn-sm btn-outline-danger" onclick="removeFromCart('${item.id}'); loadCartItems();">Ã—</button>
        <span>${item.name}</span>
...
function applyCoupon() {
  ...
  if (coupons[code]) {
    if (code === 'FREESHIP') {
      couponMessage.textContent = 'âœ“ Free shipping applied!';
      couponMessage.className = 'text-success mt-2';
      // You can modify shipping calculation here
    } else {
      couponMessage.textContent = `âœ“ ${coupons[code] * 100}% discount applied!`;
      couponMessage.className = 'text-success mt-2';
